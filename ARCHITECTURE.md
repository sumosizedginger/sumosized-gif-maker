...
*   `ffmpeg-client.js`: A lightweight, Promise-based wrapper around the raw `js/worker/ffmpeg-worker.js`. *Mocked in Vitest for CI pipelines.*
...
5.  **Render Loop**: 
    *   *Pass 1*: FFmpeg generates a dynamic color palette (`palettegen`).
    *   *Pass 2*: FFmpeg applies the palette (`paletteuse`) and composite filters to generate the final file.
